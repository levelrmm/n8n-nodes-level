...
export const devicesOperations: INodeProperties[] = [
	{
		...
		options: [
			...
			{ name: 'Get', value: 'get', action: 'Get a device', routing: { request: { method: 'GET', url: '=/devices/{{$parameter["id"]}}' } } },
		],
		default: 'list',
	},
];
export const devicesFields: INodeProperties[] = [
	{ displayName: 'Device ID', name: 'id', type: 'string', required: true, default: '', description: 'The ID of the device to retrieve.', displayOptions: { show: { resource: ['devices'], operation: ['get'] } } },
	{
		displayName: 'Options',
		name: 'options',
		type: 'collection',
	...
	{ displayName: 'Return All', name: 'returnAll', type: 'boolean', default: false, description: 'Fetch all pages automatically using cursor pagination (uses <code>starting_after</code> under the hood).', displayOptions: { show: { resource: ['devices'], operation: ['list'] } } },
	{ displayName: 'Limit', name: 'limit', type: 'number', default: 20, typeOptions: { minValue: 1, maxValue: 100 }, description: 'A limit on the number of objects to be returned. Range 1–100. Default is 20.', displayOptions: { show: { resource: ['devices'], operation: ['list'], returnAll: [false] } } },
	{
		displayName: 'Options',
		name: 'options',
		type: 'collection',
